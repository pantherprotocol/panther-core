...
import { ADVANCED_STAKE, ADVANCED_STAKE_V4, ADVANCED_UNSTAKE_V4 } from "../actions/Constants.sol";

/**
 * @title AdvancedStakeV4ActionMsgTranslator
 * @notice It processes action messages for stakes of the `advanced-v4` type.
 * @dev It "translates" the `action` in messages so that other contracts, which
 * are "unaware" of "v4" stake type, may process this type stakes exactly like
 * they process `advanced` type stakes.
 * It shall be registered with the `RewardMaster` as the "RewardAdviser" for
 * `advanced-v4` type stakes and the "ActionOracle" for `advanced` type stakes.
 * Being called `getRewardAdvice` by the RewardMaster for a message with the
 * `advanced-v4` STAKE action, it replaces the `action` to be the `advanced`
 * (but not `advanced-v4`) STAKE action.
 ...
 * it is the `advanced` (not `advanced-v4`) STAKE action.
 */
contract AdvancedStakeV4ActionMsgTranslator is IRewardAdviser {
    ...
    /// @dev To be called by the {RewardMaster} for `advanced-v4` type actions.
    /// It makes the `action` to look like the `advanced` (not "v4") type action
        ...
        returns (Advice memory)
    {
        ...
        if (action == ADVANCED_STAKE_V4) {
            ...
            require(action == ADVANCED_UNSTAKE_V4, "AMT: unsupported action");
        }
