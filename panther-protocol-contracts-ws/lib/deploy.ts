...
const [SignUpTokenGatekeeperAbi, SignUpTokenGatekeeperBin] = loadAB(
...
const [FreeForAllSignUpGatekeeperAbi, FreeForAllSignUpGatekeeperBin] = loadAB(
...
const linkPoseidonContracts = (solFilesToLink: string[], poseidonT3Address) => {
    let inputFiles = '';
...
const genProvider = (rpcUrl: string = config.get('chain.url')) => {
...
const genDeployer = (privateKey: string) => {
    return new GanacheDeployer(privateKey, config.get('chain.ganache.port'), {
        gasLimit: 10000000,
    });
};
...
const deploySignupToken = async deployer => {
    ...
    return await deployer.deploy(SignupTokenAbi, SignupTokenBin);
};
...
const deployFreeForAllSignUpGatekeeper = async (deployer, quiet = false) => {
...
const main = async () => {
    ...
    const parser = new argparse.ArgumentParser({
        description:
            'Deploy all contracts to an Ethereum network of your choice',
    });
    ...
    parser.addArgument(['-s', '--signUpToken'], {
        help: 'The address of the signup token (e.g. POAP)',
        required: false,
    });

    parser.addArgument(['-p', '--initialVoiceCreditProxy'], {
    ...
    fs.writeFileSync(addressJsonPath, JSON.stringify(addresses));
